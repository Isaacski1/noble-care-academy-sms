...
class FirestoreService {
  ...
  async upsertStudentLedger(ledger: StudentFeeLedger): Promise<void> {
    ...
    const preparedLedger = useV2 && !("createdAt" in cleanedLedger) ? { ...(cleanedLedger as object), createdAt: serverTimestamp() } : cleanedLedger;
    await setDoc(docRef, preparedLedger, {
      merge: true,
    });
  }
  ...
  async upsertStudentLedgersBulk(ledgers: StudentFeeLedger[]): Promise<void> {
    ...
    const operations = ledgers
      ...
      .map((ledger) => (batch: ReturnType<typeof writeBatch>) => {
        const ref = useV2 ? doc(firestore, `schools/${schoolId}/feeLedgers/${ledger.id}`) : doc(firestore, `student_ledgers/${ledger.id}`);
        const preparedLedger = useV2 && !("createdAt" in ledger) ? { ...(ledger as object), createdAt: serverTimestamp() } : ledger;
        batch.set(ref, preparedLedger, { merge: true });
      });
    await this.commitChunkedOperations(operations);
  }
  ...
  async recordStudentPayment(payment: StudentFeePayment): Promise<void> {
    ...
    const preparedPayment = useV2 && !("createdAt" in finalPayment) ? { ...(finalPayment as object), createdAt: serverTimestamp() } : finalPayment;
    await setDoc(docRef, preparedPayment);
