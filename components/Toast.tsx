...
const ToastItem: React.FC<{ toast: Toast; onClose: (id: number) => void }> = ({ toast, onClose }) => {
  ...
  const colorClass = toast.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : toast.type === 'error' ? 'bg-red-50 border-red-200 text-red-800' : 'bg-slate-50 border-slate-200 text-slate-800';

  return (
    <div className={`max-w-sm w-full px-4 py-3 rounded-lg border shadow-md ${colorClass} transform transition-all`}> 
      <div className="flex items-start gap-3">
        <div className="flex-1 text-sm leading-tight">{toast.message}</div>
        <button onClick={() => onClose(toast.id)} className="text-slate-400 hover:text-slate-600 ml-2">✕</button>
      </div>
    </div>
  );
};
